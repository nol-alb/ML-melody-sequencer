...
let bassFilter = new Tone.Filter({type: 'lowpass', frequency: 20000})
...
document.getElementById("delay").onclick = async () => {
  

    await leadSampler.connect(leadDelay);leadDelay.toDestination();
    leadDelay.toDestination();
...
// Machine Learning and Magenta oooh
/*
    ...
    notes: [
      {
        ...
        sequence.matrix.pattern[0][0]
        pitch: Tone.Frequency(sequencerRows[0]).toMidi(), quantizedStartStep: 0, quantizedEndStep:4}
    ],
...
document.getElementById('groove-drums').onclick = async () => {
  ...
  let original = {
    ...
    //jow many 16th note hits are present
...
let melChordsVaeLoaded = melChordsVae.initialize()
